...
pub enum Step {
    ...
    Bit { label: String, commitment: RistrettoPoint, proof: BitProof, tag: Vec<u8> },
    /// A committed difference opens to a value the verifier can reconstruct.
    Equality { label: String, commitment: RistrettoPoint, proof: OpeningProof, tag: Vec<u8> },
}
...
impl RuleAudit {
    pub fn size(&self) -> BTreeMap<&'static str, usize> {
        ...
        for step in &self.steps { *counts.entry(step.kind()).or_insert(0) += 1; }
        counts.insert("declared_range",
...
impl RuleProver {
    pub fn new() -> Self {
        RuleProver { key: Pedersen::new(b"qomm:rule:v1"), ranges: RangeCtx::new(STEP_BITS, 1) }
    }

    pub fn prove<R: RngCore + CryptoRng>(
        &self, rule: &Rule, bindings: &BTreeMap<String, i128>, context: &[u8], rng: &mut R,
        ...
        for (name, declaration) in &rule.declarations {
            ...
            wires.insert(name.clone(), Wire {
                value, blinding, commitment, interval: declaration.interval,
            });
        }

        let declared_ranges = if secret_names.is_empty() { None } else {
            let ranges = RangeCtx::new(STEP_BITS, secret_names.len().next_power_of_two());
            let mut t = transcript(&[context, b":decl"].concat());
            let (proof, commitments) = ranges.prove(&mut t, &offsets, &offset_blindings)
        ...
        Ok(RuleAudit { declared, declared_ranges, steps, outputs, output_values })
    }
    ...
    fn prove_ge_zero<R: RngCore + CryptoRng>(
        &self, wire: &Wire, label: &str, tag: Vec<u8>, steps: &mut Vec<Step>, rng: &mut R,
    ) -> Result<(), RuleError> {
        let _ = rng;
        if wire.value < 0 {
        ...
        let (proof, commitments) = self.ranges
            .prove(&mut t, &[wire.value as u64], &[wire.blinding])
            .map_err(|e| RuleError(e.into()))?;
        steps.push(Step::Range { label: label.into(), commitment: commitments[0], proof, tag });
        Ok(())
    }
    ...
    fn pin_to_one_of<R: RngCore + CryptoRng>(
        &self, left: &Wire, right: &Wire, result: &Wire, label: &str, tag: &[u8],
        steps: &mut Vec<Step>, rng: &mut R,
    ) -> Result<(), RuleError> {
        ...
        let proof = prove_product(
            &self.key, &mut transcript(&pin_tag), &gap_left.commitment,
            &scalar(gap_left.value), &gap_left.blinding,
            &scalar(gap_right.value), &gap_right.blinding, &zero_blinding, rng);
        let zero_commitment =
            self.key.commit(&scalar(gap_left.value * gap_right.value), &zero_blinding);
        steps.push(Step::Product {
            label: label.into(), a: gap_left.commitment, b: gap_right.commitment,
            c: zero_commitment, proof, tag: pin_tag,
        });
        let zero_tag = [tag, b":zero"].concat();
        let opening = prove_opening(&self.key, &mut transcript(&zero_tag),
                                    &zero_commitment, &Scalar::ZERO, &zero_blinding, rng);
        steps.push(Step::Equality {
            label: label.into(), commitment: zero_commitment, proof: opening, tag: zero_tag,
        });
        Ok(())
    }

    fn select<R: RngCore + CryptoRng>(
        &self, left: Wire, right: Wire, take_min: bool, label: &str, tag: &[u8],
        steps: &mut Vec<Step>, rng: &mut R,
    ) -> Result<Wire, RuleError> {
        let value = if take_min { left.value.min(right.value) }
                    else { left.value.max(right.value) };
        let blinding = Scalar::random(rng);
        let interval = if take_min {
            Interval { lo: left.interval.lo.min(right.interval.lo),
                       hi: left.interval.hi.min(right.interval.hi) }
        } else {
            Interval { lo: left.interval.lo.max(right.interval.lo),
                       hi: left.interval.hi.max(right.interval.hi) }
        };
        let result = Wire {
            value, blinding, commitment: self.key.commit(&scalar(value), &blinding), interval,
        };
        // The result is no worse than either input...
        let (a, b) = if take_min {
            (self.difference(&left, &result), self.difference(&right, &result))
        } else {
            (self.difference(&result, &left), self.difference(&result, &right))
        };
    ...
    fn walk<R: RngCore + CryptoRng>(
        &self, node: &Expr, wires: &BTreeMap<String, Wire>, label: &str, context: &[u8],
        steps: &mut Vec<Step>, rng: &mut R,
    ) -> Result<Wire, RuleError> {
        let n = steps.len();
        match node {
            Expr::Const(v) => Ok(Wire {
                value: *v, blinding: Scalar::ZERO,
                commitment: self.key.commit(&scalar(*v), &Scalar::ZERO),
                interval: Interval::point(*v),
            }),
            Expr::Name(name) => wires.get(name).copied()
                .ok_or_else(|| RuleError(format!("'{name}' has no wire"))),
            Expr::Neg(inner) => {
                let w = self.walk(inner, wires, label, context, steps, rng)?;
                Ok(Wire { value: -w.value, blinding: -w.blinding,
                          commitment: -w.commitment, interval: w.interval.neg() })
            }
            ...
            Expr::Mul(a, b) => {
                let left = self.walk(a, wires, label, context, steps, rng)?;
                let right = self.walk(b, wires, label, context, steps, rng)?;
                Ok(self.multiply(&left, &right, label,
                                 Self::tag(context, label, n, "mul"), steps, rng))
            }
            Expr::Compare(a, op, b) => {
                let left = self.walk(a, wires, label, context, steps, rng)?;
                let right = self.walk(b, wires, label, context, steps, rng)?;
                self.compare(&left, &right, op, label,
                             Self::tag(context, label, n, "cmp"), steps, rng)
            }
            ...
            Expr::Call(name, args) => {
                ...
                self.intrinsic(name, &evaluated, label,
                               Self::tag(context, label, n, name), steps, rng)
            }
        }
    }
    ...
    fn additive(&self, left: &Wire, right: &Wire, adding: bool) -> Wire {
        if !adding { return self.difference(left, right); }
        Wire {
            value: left.value + right.value,
            blinding: left.blinding + right.blinding,
            commitment: left.commitment + right.commitment,
    ...
    fn multiply<R: RngCore + CryptoRng>(
        &self, left: &Wire, right: &Wire, label: &str, tag: Vec<u8>,
        steps: &mut Vec<Step>, rng: &mut R,
    ) -> Wire {
        let blinding = Scalar::random(rng);
        let value = left.value * right.value;
        let proof = prove_product(
            &self.key, &mut transcript(&tag), &left.commitment,
            &scalar(left.value), &left.blinding,
            &scalar(right.value), &right.blinding, &blinding, rng);
        let commitment = self.key.commit(&scalar(value), &blinding);
        steps.push(Step::Product {
            label: label.into(), a: left.commitment, b: right.commitment,
            c: commitment, proof, tag,
        });
        Wire { value, blinding, commitment, interval: left.interval.mul(right.interval) }
    }

    fn compare<R: RngCore + CryptoRng>(
        &self, left: &Wire, right: &Wire, op: &Cmp, label: &str, tag: Vec<u8>,
        steps: &mut Vec<Step>, rng: &mut R,
    ) -> Result<Wire, RuleError> {
        let result = match op {
            ...
            Cmp::Eq | Cmp::Ne => {
                ...
                let opening = prove_opening(
                    &self.key, &mut transcript(&tag), &diff.commitment,
                    &scalar(diff.value), &diff.blinding, rng);
                steps.push(Step::Equality {
                    label: label.into(), commitment: diff.commitment,
                    proof: opening, tag: tag.clone(),
                });
                i128::from(if matches!(op, Cmp::Eq) { diff.value == 0 }
                           else { diff.value != 0 })
            }
            ...
            _ => {
                ...
                let shifted = Wire {
                    ...
                    commitment: if strict { diff.commitment - self.key.g }
                                else { diff.commitment },
    ...
    fn as_bit<R: RngCore + CryptoRng>(
        &self, value: i128, label: &str, tag: &[u8], steps: &mut Vec<Step>, rng: &mut R,
    ) -> Wire {
        let blinding = Scalar::random(rng);
        let commitment = self.key.commit(&scalar(value), &blinding);
        let bit_tag = [tag, b":bit"].concat();
        let proof = prove_bit(&self.key, &mut transcript(&bit_tag), &commitment,
                              value == 1, &blinding, rng);
        steps.push(Step::Bit { label: label.into(), commitment, proof, tag: bit_tag });
        Wire { value, blinding, commitment, interval: Interval { lo: 0, hi: 1 } }
    }

    fn conjunction<R: RngCore + CryptoRng>(
        &self, parts: &[Expr], wires: &BTreeMap<String, Wire>, label: &str,
        context: &[u8], steps: &mut Vec<Step>, rng: &mut R,
    ) -> Result<Wire, RuleError> {
    ...
    fn intrinsic<R: RngCore + CryptoRng>(
        &self, name: &str, args: &[Wire], label: &str, tag: Vec<u8>,
        steps: &mut Vec<Step>, rng: &mut R,
    ) -> Result<Wire, RuleError> {
        match (name, args) {
            ...
            ("clamp", [value, lo, hi]) => {
                let lowered = self.select(*value, *lo, false, label,
                                          &[tag.as_slice(), b":lo"].concat(), steps, rng)?;
                self.select(lowered, *hi, true, label,
                            &[tag.as_slice(), b":hi"].concat(), steps, rng)
            }
    ...
    fn signed<R: RngCore + CryptoRng>(
        &self, side: &Wire, magnitude: &Wire, label: &str, tag: Vec<u8>,
        steps: &mut Vec<Step>, rng: &mut R,
    ) -> Wire {
        let blinding = Scalar::random(rng);
        let product_value = side.value * magnitude.value;
        let proof = prove_product(
            &self.key, &mut transcript(&tag), &side.commitment,
            &scalar(side.value), &side.blinding,
            &scalar(magnitude.value), &magnitude.blinding, &blinding, rng);
        let product_commitment = self.key.commit(&scalar(product_value), &blinding);
        steps.push(Step::Product {
            label: label.into(), a: side.commitment, b: magnitude.commitment,
            c: product_commitment, proof, tag,
        });
        ...
        Wire {
            ...
            interval: Interval { lo: -magnitude.interval.hi, hi: magnitude.interval.hi },
        }
    }
}
...
impl RuleVerifier {
    pub fn new() -> Self {
        RuleVerifier { key: Pedersen::new(b"qomm:rule:v1"), ranges: RangeCtx::new(STEP_BITS, 1) }
    }

    /// Nothing here opens anything. Every check is against a commitment the
    /// verifier already holds or reconstructs.
    pub fn verify(&self, rule: &Rule, audit: &RuleAudit, context: &[u8])
        -> Result<(), String> {
        ...
        if let Some((proof, commitments, names)) = &audit.declared_ranges {
            ...
            for name in names {
                let declaration = rule.declarations.get(name)
                    .ok_or_else(|| format!("{name}: no such declaration"))?;
                let commitment = audit.declared.get(name)
                    .ok_or_else(|| format!("{name}: no commitment"))?;
                expected.push((commitment - self.key.g * scalar(declaration.interval.lo))
                              .compress());
            }
        ...
        for step in &audit.steps {
            let ok = match step {
                Step::Product { a, b, c, proof, tag, .. } =>
                    verify_product(&self.key, &mut transcript(tag), a, b, c, proof),
                Step::Range { commitment, proof, tag, .. } =>
                    self.ranges.verify(&mut transcript(tag), proof,
                                       std::slice::from_ref(commitment)),
                Step::Bit { commitment, proof, tag, .. } =>
                    verify_bit(&self.key, &mut transcript(tag), commitment, proof),
                Step::Equality { commitment, proof, tag, .. } =>
